...
func main() {
	...
	// 初始化服务注册中心
	serviceRegistry := internalregistry.NewServiceRegistry(cfg.HealthCheckInterval)
	serviceRegistry.StartHealthChecks()

	// 初始化处理器
	serviceHandler := internalhandlers.NewServiceHandler(serviceRegistry)
	...
	// 设置路由（已移至 routers 包）
	routers.SetupRoutes(router, cfg, serviceHandler)
